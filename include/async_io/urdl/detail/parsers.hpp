#ifndef __ASYNC_URDL_DETAIL_PARSERS_HPP
#define __ASYNC_URDL_DETAIL_PARSERS_HPP
...
namespace urdl 
{
	namespace detail 
	{
		...
		inline bool string_equal(const std::string& a, const std::string& b)
		{
		...
		inline void check_header(const std::string& name, const std::string& value, std::string& content_type, std::size_t& content_length, std::string& location)
		{
			if( string_equal(name, "Content-Type") )
				content_type = value;
			else if (string_equal(name, "Content-Length"))
				content_length = std::atoi(value.c_str());
			else if (string_equal(name, "Location"))
				location = value;
		}
