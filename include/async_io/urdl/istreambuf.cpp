...
namespace urdl 
{
	...
	istreambuf* istreambuf::open(const url& u)
	{
		...
		if (!body_->read_stream_.is_open())
			body_->error_ = std::make_error_code(std::errc::timed_out);
	...
	std::streambuf::int_type istreambuf::underflow()
	{
		if (gptr() == egptr())
		{
			...
			detail::istreambuf_read_handler rh = { body_->error_, bytes_transferred, body_->timer_ };
